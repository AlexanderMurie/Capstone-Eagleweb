...
plotRiskMap <- function(pred)
{
  ...
  jpeg("ipplot1.jpg", width = 575, height = 1061) #open a png file
...
readUserCSVFile <- function() #Read via CSV
{
  ...
  assign("csvfile", userInput, envir = .GlobalEnv)
  #find row length
  ...
  for (rowItem in userInput$Name) # iterate for each row item in the first column called "Name"
  {
    ...
    else if (rowItem == "srtmsd"){

      focalFilePath <- userInput$Path[myRowCounter] #rowLength - 2 will always store the srtm tags
      focalFilePath <- paste("", focalFilePath, sep = "")
      #print(focalFilePath, max.levels = 0)  #TRACER
      focalFileList[[focalCount]] <- raster(focalFilePath)
      #print(focalFileList)
      focalCount = focalCount + 1
    }
